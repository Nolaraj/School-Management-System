...
class Clock:
    def __init__(self, bottom_frame):
        self.time2 = datetime.datetime.now().strftime('%H:%M')
        self.mFrame = bottom_frame
    ...
    def changeLabel(self):
        ...
        self.mFrame.after(60000, self.changeLabel)


school_name = "ABC Boarding School"
user = "Login User" # todo after making login window
options_teachers = ("Hari", "Shyam", "Krishna")  # todo - Should be made to extract all the staffs name and teachers name to store in the tuple
...
def framework():
    def log_handler(text1, text2, text3, text4, text5, text6):
        ...
        log_sheet.cell(row=row_number + 1, column=1).value = row_number - 2
    ...
    def academic_input(event):
        ...
        def excel_input(event):
            ...
            def generate_sheets(event):
                            ...
                            try:
                                ...
                                for i in range(1, 1 + g_sheet.max_row):
                                    if g_sheet.cell(row=i, column=1).value != None:
                                        ...
                                        pass
                                w_sheet.merge_cells(start_row=2, end_row=2, start_column=1, end_column=w_sheet.max_column)
                                w_sheet.cell(row=2, column=3).value = school_name
                                w_sheet.cell(row=1, column=1).value = "Generated By:"
                                w_sheet.cell(row=1, column=2).value = user
                                w_sheet.cell(row=1, column=3).value = "Generated On:"
                                w_sheet.cell(row=1, column=4).value = date_time
                                w_sheet.cell(row=3, column=2).value = "Class"
                                w_sheet.cell(row=3, column=3).value = g_class1
                                w_sheet.cell(row=4, column=2).value = "Submitted To:"
                                w_sheet.cell(row=4, column=3).value = g_teacher.get()
                ...
                existing_record = Label(excel_2, text="Submitted To:", font=("verdana", 10),
                                        fg="black", border=1, relief=GROOVE)
                existing_record.grid(row=4, column=0, sticky="ew")

                g_submisssion = ttk.Combobox(excel_2, state="readonly", values=options_teachers,
                                       font=("verdana", 10), textvariable=g_teacher)
                g_submisssion.grid(row=5, column=0, columnspan=4, sticky="ew")



                existing_record1 = Button(excel_2, text="Submit", font=("verdana", 12),
                                          fg="black", bg="springgreen2", border=1, relief=GROOVE)
                existing_record1.grid(row=6, column=0, sticky="ew")
        ...
        def courses_registration(event):
            ...
            def register_data(event, c_name):
                ...
                def confirm(event, reg_list):
                    ...
                    w_reg.save(filename)
                    text = f"{filename[0:-5]} Course for year {entry_year} has successfuly been registered in {date_time}\n"
                    act_text.configure(state='normal')
                    act_text.insert('1.0', f'\n {text}')
                    act_text.configure(state='disabled')
                    text1 = f"Course Registration of {filename[0:-5]} for year {entry_year}"
                    log_handler("Courses Registration", text1, "New Registration", date, time, "Anonymous")
    ...
    def new_registration_event1(event):
        def teacher_to_db(event):
            ...
            def composite_writer():
                ...
                text1 = f"Creation of {teacher_name.get()} + {teacher_age.get()}.xlsx"
                log_handler("Teacher Database", text1, "New Registration", date, time, "Anonymous")
    ...
    #  welcome_image1.pack()
    # ------------
    # Attractive Font = bankGothic Md BT and bankGothic lt BT
...
def intro():
    ...
    intro_frame = Frame(root1)
    intro_frame.pack(side=TOP) #place(x=20, y=20)
    intro_text = f"Welcome to the School Management System of {school_name} \nDeveloper : Nolaraj Poudel \nVersion - 1.0"
    intro_label = scrolledtext.ScrolledText(root1, font=("bankGothic lt BT", 16), width=50, height=5)
    intro_label.pack(fill=X)

    intro_label.configure(state='normal')
    intro_label.insert(INSERT, intro_text)
    intro_label.configure(state='disabled')
    intro_frame.after(20000, framework())
